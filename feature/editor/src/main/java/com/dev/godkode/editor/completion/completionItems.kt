...
abstract class GodKodeCompletionItem(
...
) : GodKodeCompletionItem(completionKind, label, desc) {
...
class AICompletionItem(
    ...
    desc: CharSequence?,
    private val commitText: String,
) : GodKodeCompletionItem(CompletionItemKind.AI_GENERATED, label, desc) {
    override fun performCompletion(editor: CodeEditor, text: Content, line: Int, column: Int) {
        text.insert(line, column, commitText)
    }
}
...
) : GodKodeCompletionItem(CompletionItemKind.SNIPPET, label, desc) {
